#!/bin/sh
# Keep DiaRouter resident, so a link never waits for a cold start.
#
...
[ -n "$APP" ] || APP=$(ls -d /Applications/DiaRouter.app "$HOME/Applications/DiaRouter.app" 2>/dev/null | head -1)
[ -n "$APP" ] || { echo "DiaRouter.app not found — drag it to Applications first" >&2; exit 1; }
...
echo "DiaRouter will start at login, from $APP"
